...
app.get("/api/debug", async (request, reply) => {
  ...
  const out: any = { profile, pdir, exists: fs.existsSync(pdir) };
  try{
    out.singleCsv = fs.existsSync(singleCsv);
    out.historyDir = fs.existsSync(hdir) && fs.statSync(hdir).isDirectory();
    if (out.historyDir) {
      out.files = safeArr(fs.readdirSync(hdir)).filter(f=>f.toLowerCase().endsWith(".csv")).length;
    }
  }catch{}
  reply.send(out);
});
